...
VSTEffect::VSTEffect(std::string& path, int p_samplerate, int p_blocksize) : VSTBase(path)
{
	...
	/////VSTi setup
	memset(&midiOutEvents, 0, sizeof(VstEvents) * 2);
	
	midiOutEvents.numEvents = 2;
	for (int i = 0; i < 2; i++)
	{
		VstMidiEvent mEvent;
		memset(&mEvent, 0, sizeof(VstMidiEvent));

		mEvent.type = kVstMidiType;
		mEvent.byteSize = sizeof(VstMidiEvent);
		mEvent.flags = 0;
		mEvent.detune = 0;
		mEvent.noteOffVelocity = (char)0;
		mEvent.reserved1 = 0;
		mEvent.reserved2 = 0;
		mEvent.midiData[3] = 0;
		mEvent.noteOffset = 0;
		mEvent.midiData[1] = 60;
		mEvent.midiData[2] = 80;
		

		midiEvent.push_back(mEvent);
		midiOutEvents.events[i] = (VstEvent*)&midiEvent[i];
	}
	midiEvent[0].midiData[0] = 0x91;
	midiEvent[0].deltaFrames = 0;
	midiEvent[1].midiData[0] = 0x91;
	midiEvent[0].deltaFrames = 256;
...
void VSTEffect::processAudio(float* audioThrough, long numFrames, int numUnityChans)
{
	...
	processMidi(plugin, &midiOutEvents);
...
void VSTEffect::processMidi(AEffect *plugin, VstEvents *events) {
